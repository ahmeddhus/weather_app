...
void main() {
  test('repository with mocked http client, success', () async {
    ...
    when(() => mockHttpClient.get(api.weather('Egypt')))
        .thenAnswer((_) => Future.value(http.Response(encodedWeatherJsonResponse, 200)));

    final weather = await weatherRepository.getWeather(city: 'Egypt');
  ...
  test('repository with mocked http client, failure', () async {
    ...
    expect(() => weatherRepository.getWeather(city: 'Egypt'), throwsA(isA<APIException>()));
  });
}
